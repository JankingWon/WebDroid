...
import cn.janking.webDroid.util.ByteUtils;
...
public class StringChunk {
	...
	public LinkedList<String> stringContentList;
	
	public byte[] getByte(List<String> strList){
	...
	public static StringChunk createChunk(byte[] byteSrc, int stringChunkOffset) throws UnsupportedEncodingException {
		...
		chunk.stringContentList = new LinkedList<>();
...
//		}
	...
	private byte[] getStrListByte(List<String> strList){
		byte[] src = new byte[0];
//		ArrayList<String> stringContentList = convertStrList(strList);
		for(int i=0;i<strList.size();i++){
			byte[] tempAry = new byte[0];
			short len = (short)(strList.get(i).length());
			byte[] lenAry = Utils.shortToByte(len);
			tempAry = Utils.addByte(tempAry, lenAry);
			tempAry = Utils.addByte(tempAry, ByteUtils.string2Byte(strList.get(i)));
//			tempAry = Utils.addByte(tempAry, strList.get(i).getBytes());
